class FeedsController < ApplicationController
  ...
  def feed
    ...
    @timeline_events = TimelineEvent.paginate(:all, :conditions=>"icon_file_name is not null",:joins=>"INNER JOIN users on users.id = timeline_events.actor_id",:order=> 'created_at DESC', :page=>params[:page], :per_page=> 5)
