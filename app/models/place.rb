  ...
  def self.search_places_map(params, current_user)
      ...
      conditions = search_criteria[4]
       ...
       if !use_activity && !use_place_location #just user (gender sex)
         ...
         results = Place.paginate(:select => "places.latitude, places.longitude, places.name, places.address, places.neighborhood, places.category,count(PA.activity_id)", :order => "count(user_id) DESC", :joins => "inner join place_activities PA on PA.place_id = places.id inner join user_place_activities UPA on PA.id = UPA.place_activity_id inner join activities on activities.id = PA.activity_id inner join users on users.id = UPA.user_id",:group => 'places.latitude, places.longitude, places.name, places.address, places.neighborhood, places.category', :order => "count(PA.activity_id) DESC", :conditions => conditions, :page => params[:page], :per_page => 100)
       end
