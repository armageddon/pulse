class UserPlaceActivity < ActiveRecord::Base
  ...
  def self.search_user_place_activities(params, current_user)
      ...
      if !use_activity && !use_place_location #just user (gender sex)
        ...
        results = UserPlaceActivity.paginate(:select => "place_activities.activity_id,place_activities.place_id, count(user_id) as users_count", :order => "count(user_id) DESC", :joins => "inner join place_activities  on place_activities.id = user_place_activities.place_activity_id inner join users on users.id = user_place_activities.user_id",:group => 'place_activities.activity_id,place_activities.place_id', :conditions => conditions, :page => params[:page], :per_page => 10, :order => "count(user_id) DESC")
      end
      if !use_activity && use_place_location
        logger.debug("location")
        results = UserPlaceActivity.paginate(:select => "place_activities.activity_id,place_activities.place_id, count(user_id) as users_count", :order => "count(user_id) DESC", :joins => "inner join place_activities  on place_activities.id = user_place_activities.place_activity_id inner join users on users.id = user_place_activities.user_id inner join places on place_activities.place_id = places.id",:group => 'place_activities.activity_id,place_activities.place_id', :order => "count(user_id) DESC", :conditions => conditions, :page => params[:page], :per_page => 10)
      end
      if use_activity && !use_place_location
        logger.debug("activities")
        results = UserPlaceActivity.paginate(:select => "place_activities.activity_id,place_activities.place_id, count(user_id) as users_count", :order => "count(user_id) DESC", :joins => "inner join place_activities  on place_activities.id = user_place_activities.place_activity_id inner join users on users.id = user_place_activities.user_id inner join activities on place_activities.activity_id = activities.id",:group => 'user_place_activities.activity_id,user_place_activities.place_id', :order => "count(user_id) DESC", :conditions => conditions,:page => params[:page], :per_page => 10) 
      end
      if use_activity && use_place_location
        logger.debug("location and activities")
        results = UserPlaceActivity.paginate(:select => "place_activities.activity_id,place_activities.place_id, count(user_id) as users_count", :order => "count(user_id) DESC", :joins => "inner join place_activities  on place_activities.id = user_place_activities.place_activity_id inner join users on users.id = user_place_activities.user_id inner join places on place_activities.place_id = places.id inner join activities on place_activities.activity_id = activities.id", :order => "count(user_id) DESC",:group => 'user_place_activities.activity_id,user_place_activities.place_id', :conditions => conditions,:page => params[:page], :per_page => 10) 
      end
