...
module LaterDude
  ...
  class Calendar
    ...
    def initialize(year, month, options={}, &block)
      ...
      start_date = Date.civil(@year, @month, Date.today.day)
    ...
    def to_html
      <<-EOF
        <table class="#{@options[:calendar_class]}" cellspacing="5px">
          <thead>
            #{show_month_names}
            #{show_day_names}
          </thead>
    ...
    def show_current_month
      returning "" do |output|
        @days.first.upto(@days.first + 13.days) { |d| output << show_day(d) }
      end
    end
    ...
    def show_day(day)
      ...
      returning content_tag(:td, content, options) do |output|
        ...
        # end
        if day == @days.first + 6.days # opening and closing tag for the first and last week are included in #show_days
          output << "</tr><tr>" # close table row at the end of a week and start a new one
        end
